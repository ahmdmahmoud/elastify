...
def _create_parser():
    ...
    On index ntcir_titles, use strategy 'fulltext' as gold standard
    python3 querify.py -I ntcir_titles -g tfidf4gold


    ntcir_titles/_all/title.TFIDF
    ...
    parser.add_argument('-I', '--gold-index', dest='gold_index',
                        default='economics', help="Specify the gold-standard's\
                        elasticsearch index [economics]")
    parser.add_argument('-d', '--doc-type', dest='doc_type',
                        default='publication',
                        help="Specify the elasticsearch document type,\
                        [publication]")
    parser.add_argument('-D', '--gold-doc-type', default='publication',
                        dest='gold_doc_type',
                        help="Specify the gold-standard's elasticsearch\
                        document type [publication]")

    parser.add_argument('-g', '--goldstandard', type=str,
                        default='fulltext', choices=utils.FIELDS,
                        help='specify the goldstandard for the evaluation')
...
def main():
    ...
    try:
        ...
        # gold standard is list of lists
        # goldstandard = [utils.batched(args.batches, doc_ids(docs)) for
        #                 docs in utils.perform_queries(args.gold_index,
        #                                               args.gold_doc_type,
        #                                               None,  # no prefix
        #                                               gold_strategy,
        #                                               querystrings,
        #                                               size=gold_count,
        #                                               source=False)]
        goldstandard = [defaultdict(int, utils.batched(args.batches,
                                                       utils.doc_ids(docs))) for
                        docs in utils.execute_multisearch(
                            args.gold_index,
                            gold_strategy,
                            querystrings,
                            size=gold_count,
                        doc_type=args.gold_doc_type)]
        # list of dicts [dict] : list index corresponds to query id, dictionary keys correspond to document identifiers
        # [{"gakkai-somethign" : 1}, {"gakkai-eles":1}]
        # defaultdict(int) -> if key not present return 0
        #print('gs:', goldstandard)
